...
-- Same, but without assumption we're connected to a server
{-
testProg2 : Program () (const Void)
testProg2 = do mpid <- Fork mathsServer
               cpid <- Fork (countServer 0)
               putStr "Number> "
               x <- getLine
 
               Just fac <- TrySend mpid (Factorial (cast (trim x)))
                   | Nothing => do putStrLn "Maths server died"
                                   Disconnect mpid
                                   Disconnect cpid
               putStrLn ("Answer : " ++ show fac)
 
               Just secs <- TrySend cpid GetIdle
                   | Nothing => do putStrLn "Count server died"
                                   Disconnect mpid
                                   Disconnect cpid
               putStrLn (show secs ++ " seconds taken")
